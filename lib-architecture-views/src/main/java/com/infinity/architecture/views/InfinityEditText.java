...
public class InfinityEditText extends AppCompatEditText {
    ...
    private boolean passToggleEnabled;
    ...
    private boolean monetaryMaskShowSign      = true;
    ...
    // Mask edit formatter listener to dispatch to user watchers listeners
    private final TextWatcher maskEditTextWatcher = new TextWatcher() {
        ...
        public void afterTextChanged(Editable s) {
            if (s.length() == 1 || s.length() == 0) {
                configureInputTypeForPassToggleVariant();
            }
            if (textChangeListeners != null) {
                for (TextWatcher cursor : textChangeListeners) {
    ...
    public InfinityEditText(Context context) {
        this(context, null);
    }

    public InfinityEditText(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.infinityEditTextStyle);
    }

    public InfinityEditText(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        super.addTextChangedListener(maskEditTextWatcher);

        defaultBg = getBackground();

        TypedArray taAttrs = getContext().obtainStyledAttributes(attrs, R.styleable.InfinityEditText, defStyleAttr, R.style.InfinityEditTextStyle);

        bgColor                    = taAttrs.getColor(R.styleable.InfinityEditText_bg_color, Color.TRANSPARENT);
        bgColorFocused             = taAttrs.getColor(R.styleable.InfinityEditText_bg_color_focused, Color.TRANSPARENT);
        bgBorderColor              = taAttrs.getColor(R.styleable.InfinityEditText_bg_border_color, Color.TRANSPARENT);
        bgBorderColorFocused       = taAttrs.getColor(R.styleable.InfinityEditText_bg_border_color_focused, Color.TRANSPARENT);
        bgBorderWidth              = taAttrs.getDimension(R.styleable.InfinityEditText_bg_border_width, 0);
        bgBorderWidthFocused       = taAttrs.getDimension(R.styleable.InfinityEditText_bg_border_width_focused, -1);
        bgRadius                   = taAttrs.getInteger(R.styleable.InfinityEditText_bg_radius, 0);
        bgRadiusFocused            = taAttrs.getInteger(R.styleable.InfinityEditText_bg_radius_focused, 0);
        bgRadiusTopLeft            = taAttrs.getInteger(R.styleable.InfinityEditText_bg_radius_top_left, 0);
        bgRadiusTopLeftFocused     = taAttrs.getInteger(R.styleable.InfinityEditText_bg_radius_top_left_focused, 0);
        bgRadiusTopRight           = taAttrs.getInteger(R.styleable.InfinityEditText_bg_radius_top_right, 0);
        bgRadiusTopRightFocused    = taAttrs.getInteger(R.styleable.InfinityEditText_bg_radius_top_right_focused, 0);
        bgRadiusBottomLeft         = taAttrs.getInteger(R.styleable.InfinityEditText_bg_radius_bottom_left, 0);
        bgRadiusBottomLeftFocused  = taAttrs.getInteger(R.styleable.InfinityEditText_bg_radius_bottom_left_focused, 0);
        bgRadiusBottomRight        = taAttrs.getInteger(R.styleable.InfinityEditText_bg_radius_bottom_right, 0);
        bgRadiusBottomRightFocused = taAttrs.getInteger(R.styleable.InfinityEditText_bg_radius_bottom_right_focused, 0);
        bgShapeType                = EnBgShapeType.values()[taAttrs.getInt(R.styleable.InfinityEditText_bg_shape_type, DrawableHelper.SHAPE_TYPE_RECTANGLE)];
        maskType                   = MaskEditType.values()[taAttrs.getInt(R.styleable.InfinityEditText_mask_type, MaskEditType.NONE.ordinal())];
        monetaryMaskDecimalPlaces  = taAttrs.getInt(R.styleable.InfinityEditText_mask_monetary_decimal_places, 2);
        monetaryMaskLocale         = taAttrs.getInt(R.styleable.InfinityEditText_mask_monetary_locale, 0);
        monetaryMaskShowSign       = taAttrs.getBoolean(R.styleable.InfinityEditText_mask_monetary_show_currency, true);
        passToggleEnabled          = taAttrs.getBoolean(R.styleable.InfinityEditText_password_toggle_enabled, false);
        allowPassMaskLength        = taAttrs.getBoolean(R.styleable.InfinityEditText_mask_allow_pass_mask_length, false);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_UP) {
            ...
            // Log.d("InfinityEditText", "rawX:" + event.getX() + " - getLeft():" + getLeft() + " - drawableLeftWidth:" + compoundDrawables[DrawablePos.LEFT.ordinal()].getBounds().width() + " - getCompoundDrawablePadding: " + getCompoundDrawablePadding());

//            if (compoundDrawables[DrawablePos.TOP.ordinal()] != null) {
//                 Log.d("InfinityEditText", "top:" + getTop() + " - editY: " + getY() + " - height: " + getHeight() + " - EVENT -" + "y:" + event.getY() + " - rawY:" + event.getRawY() + " - height:" + compoundDrawables[DrawablePos.TOP.ordinal()].getBounds().height() + " - drawablePadding:" + getCompoundDrawablePadding());
//                // Log.d("InfinityEditText", "bottom:" + getBottom() + " - editY: " + getY() + " - height: " + getHeight() + " - EVENT -" + "y:" + event.getY() + " - rawY:" + event.getRawY() + " - height:" + compoundDrawables[DrawablePos.BOTTOM.ordinal()].getBounds().height() + " - drawablePadding:" + getCompoundDrawablePadding());
//                // Log.d("InfinityEditText", "left:" + getLeft() + " - editX: " + getX() + " - width: " + getWidth() + " - EVENT -" + "x:" + event.getX() + " - rawX:" + event.getRawX() + " - width:" + compoundDrawables[DrawablePos.BOTTOM.ordinal()].getBounds().width() + " - drawablePadding:" + getCompoundDrawablePadding());
//            }

            if (passToggleEnabled) {
    ...
    /**
     * Configure the pass toggle
     */
    private void configurePassToggle(boolean disablingPassToggle) {
        Drawable[] compoundDrawables = getDrawables();
        if (disablingPassToggle) {
            compoundDrawables[EnDrawablePos.RIGHT.ordinal()] = null;
        }
        if (passToggleEnabled) {
            // SALVA OS DRAWABLES
            toggleOn  = ResourcesCompat.getDrawable(getResources(), R.drawable.ic_visibility_24dp, null);
            toggleOff = ResourcesCompat.getDrawable(getResources(), R.drawable.ic_visibility_off_24dp, null);

            // CONFIGURA O TOGGLE COMO DISABLED
            setCompoundDrawablesRelativeWithIntrinsicBounds(compoundDrawables[EnDrawablePos.LEFT.ordinal()], compoundDrawables[EnDrawablePos.TOP.ordinal()], toggleOff, compoundDrawables[EnDrawablePos.BOTTOM.ordinal()]);
        } else {
            // CONFIGURA O TOGGLE COMO DISABLED
            setCompoundDrawablesRelativeWithIntrinsicBounds(compoundDrawables[EnDrawablePos.LEFT.ordinal()], compoundDrawables[EnDrawablePos.TOP.ordinal()], compoundDrawables[EnDrawablePos.RIGHT.ordinal()], compoundDrawables[EnDrawablePos.BOTTOM.ordinal()]);
        }
    }

    /**
     * Toggle the pass toggle
     */
    private void toggleThePassToggle() {
        Drawable[] compoundDrawables = getDrawables();

        if (compoundDrawables[EnDrawablePos.RIGHT.ordinal()] == toggleOff) {
            setCompoundDrawablesRelativeWithIntrinsicBounds(compoundDrawables[EnDrawablePos.LEFT.ordinal()], compoundDrawables[EnDrawablePos.TOP.ordinal()], toggleOn, compoundDrawables[EnDrawablePos.BOTTOM.ordinal()]);
        } else if (compoundDrawables[EnDrawablePos.RIGHT.ordinal()] == toggleOn) {
            setCompoundDrawablesRelativeWithIntrinsicBounds(compoundDrawables[EnDrawablePos.LEFT.ordinal()], compoundDrawables[EnDrawablePos.TOP.ordinal()], toggleOff, compoundDrawables[EnDrawablePos.BOTTOM.ordinal()]);
        }
    ...
    /**
     * Update/Configure input type for pass toggle
     */
    private void configureInputTypeForPassToggleVariant() {
        if (passToggleEnabled) {
            Drawable[] compoundDrawables = getDrawables();

            if ((compoundDrawables[EnDrawablePos.RIGHT.ordinal()] == toggleOn || (getText() == null || getText().length() == 0)) && (getInputType() != InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD)) {
                setInputType(InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD);
            } else if (compoundDrawables[EnDrawablePos.RIGHT.ordinal()] == toggleOff && (getInputType() != (InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD)) && getText() != null && getText().length() > 0) {
    ...
    /**
     * Check if pass toggle is enabled
     *
     * @return  true=Enabled, false=Disabled
     */
    public boolean isPassToggleEnabled() {
        return passToggleEnabled;
    }

    /**
     * Enable/Disable pass toggle
     *
     * @param passToggleEnabled true=Enable, false=Disable
     */
    public void setPassToggleEnabled(boolean passToggleEnabled) {
        boolean isDisablingPassToggle = !passToggleEnabled && this.passToggleEnabled;
        this.passToggleEnabled = passToggleEnabled;
        configurePassToggle(isDisablingPassToggle);
    }
